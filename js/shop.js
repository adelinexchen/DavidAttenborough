...
if (localStorage.getItem("itemsPurchased") == null) {
  localStorage.setItem("itemsPurchased", "0");
}
  ...
  coins.forEach((coin) => {
    shopItem.addEventListener("click", () => {
      ...
      document.getElementById("confirm_alert").addEventListener("click", () => {
        if (
          Number(localStorage.getItem("balance")) < Number(coin.innerHTML) &&
          ...
          localStorage.setItem(
            Number(localStorage.getItem("itemsPurchased")),
            shopItem.id
          );
          localStorage.setItem(
            "itemsPurchased",
            Number(localStorage.getItem("itemsPurchased")) + 1
          );
          localStorage.setItem(
            "balance",
            String(
